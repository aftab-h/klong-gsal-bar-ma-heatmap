...
const array_to_true_object = (arr) =>
  Object.fromEntries(arr.map((x) => [x, true]));
...
class EwtsConverter {
  ...
  static suffixes = array_to_true_object([
    "'",
    "g",
    "ng",
    "d",
  ...
  superscript(sup, below) {
    let s = this.constructor.superscripts[sup];
    return s && s[below];
  }
  ...
  subscript(sub, above) {
    let s = this.constructor.subscripts[sub];
    return s && s[above];
  }
  ...
  prefix(pref, after) {
    let s = this.constructor.prefixes[pref];
    return s && s[after];
  }
  ...
  suff2(suff, before) {
    let s = this.constructor.suff2[suff];
    return s && s[before];
  }
  ...
  constructor({
    ...
    if (check_strict && !check)
      throw new Error("Cannot have 'check_strict' without 'check'.");
    this.check = check;
  ...
  to_unicode(str, keep = null) {
    ...
    let tokens = str
      .split(this.constructor.tokens_regex)
      .filter((x) => x !== "");
    tokens.push(null);
    let i = 0,
      t;
  ...
  _to_unicode_one_tsekbar(tokens, i) {
    ...
    while (t !== null && (this.vowel(t) || this.consonant(t)) && !visarga) {
      ...
      ({
        ...
        warns: these_warns,
        visarga,
      } = this._to_unicode_one_stack(tokens, i));
      ...
      if (state === "PREFIX" && cons) {
        ...
        if (this.is_suff2(cons)) {
          if (!this.suff2(cons, prev_cons)) {
            warns.push(
              `"Second suffix "${cons}" does not occur after "${prev_cons}".`,
            );
          }
        } else {
          // handles pa'm, pa'ng
    ...
    if (this.check && !warns.length && check_root && root_idx !== null) {
      ...
      if (
        ...
        this.suff2("s", consonants[1]) &&
        consonants[2] === "s"
      ) {
        let cc = consonants.join(""),
        ...
        for (let z of [0, 1]) {
          ...
          if (t2 !== null && this.is_subscript(t2)) {
            ...
            if (this.check_strict && !plus) {
              let prev = this._consonant_string_backwards(
                tokens,
                i - 1,
                orig_i,
              );
      ...
      if (t === "+") {
        ...
        if (this.check) {
          if (!this.vowel(t) && vowel_sign) {
            ...
            warns.push(
              `Cannot subjoin a-chen "${t}" after vowel "${vowel_sign}" in same stack.`,
            );
          }
        }
    ...
    while (t !== null && this.finals(t)) {
      ...
      if (final_found[klass] !== undefined) {
        if (final_found[klass] === t) {
          ...
          warns.push(
            `Cannot have "${t}" and "${final_found[klass]}" applied to the same stack.`,
          );
        }
      } else {
        final_found[klass] = t;
  ...
  to_ewts(str) {
    ...
    ITER: while (i < len) {
      ...
      if (
        o &&
        (t !== " " ||
          (!this.pass_through && !this._followed_by_nontibetan(str, i)))
      ) {
        out += o;
        i++;
        units++;
      ...
      if (tchar >= 0x0f00 && tchar <= 0x0fff) {
        ...
        if (this.tib_subjoined(t) || this.tib_vowel(t) || this.tib_final(t)) {
          this.warn(
            `line ${line}: Tibetan sign ${o} needs a top symbol to attach to.`,
          );
        }
        continue ITER;
      }
  ...
  _to_ewts_one_tsekbar(str, len, i) {
    ...
    ITER: while (true) {
      let {
        ...
        stack,
        warns: these_warns,
      } = this._to_ewts_one_stack(str, len, i);
      stacks.push(stack);
      warns = warns.concat(these_warns);
      i += toks;
    ...
    if (
      ...
      this.is_suffix(stacks[last].single_cons)
    ) {
    ...
    if (
      ...
      if (amb) {
        ...
        warns.push(
          `Ambiguous syllable found: root consonant not known for "${str}".`,
        );
        // make it up...
        root = 1;
      }
    ...
    if (
      stacks[0].prefix &&
      this.tib_stack(stacks[0].single_cons + "+" + stacks[1].cons_str)
    ) {
      stacks[0].dot = true;
    }
      ...
      if ((o = this.tib_subjoined(t))) {
        ...
        if (o === "^") {
          ...
          if (stack.finals_found[klass] !== undefined) {
            warns.push(
              `Final sign "${o}" should not combine with final sign "${stack.finals_found[klass]}".`,
            );
          } else {
